...
batch_path = 'experiments/Gridworld/A4/P7/sampled_buffer.pkl.xz'
...
def main():
    ...
    seq_pairs = batch['seq_pairs']
    indep_seqs = batch['indep_seqs']
    traj = []
    for seq_pair in seq_pairs:
        traj.append(seq_pair.x)
    traj = jnp.array(traj)
    indep_traj = indep_seqs.x
    print(traj.shape, indep_traj.shape)
    ...
    for alg in chk_path:
        for i in idx:
            ...
            if 'drqn' in alg:
                ...
                phi_t3 = agent.phi(agent.state.params, indep_traj)[0][:,-1]
            else:
                phi_t1 = agent.phi(agent.state.params, traj[:, 0, -1]).out 
                phi_t2 = agent.phi(agent.state.params, traj[:, 1, -1]).out 
                phi_t3 = agent.phi(agent.state.params, indep_traj[:, -1]).out
...
def state_awareness(phi_t1, phi_t2, phi_t3):
    ...
    dist_random = jnp.linalg.norm(phi_t1 - phi_t3, axis=1)

    return 1 - (jnp.sum(dist_same)/jnp.sum(dist_random)).item()
