...
if __name__ == "__main__":
    ...
    def add_entry(df, env, alg, x, y, low, high):
        new_entry = pd.DataFrame([[env, alg, x, y, low, high]], columns=df.columns)
        return pd.concat([df, new_entry], ignore_index=True)

    for env, sub_results in results.groupby_directory(level=2):
        for alg_result in sub_results:
            alg = alg_result.filename
            if alg not in ["TMaze-DRQN-use-all-steps-2-16", "TMaze-DRQN-fta-use-all-steps-2-16", "TMaze-DQN-fta", "TMaze-DQN"]:
                continue
            print(alg)
            ...
            collector_df = add_entry(collector_df, env, alg, xs[0], res.sample_stat, res.ci[0], res.ci[1])

        collector_df.to_csv(f"{path}/learning_curve.csv")