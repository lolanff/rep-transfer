...
subsampled_goals = ci_df['Goal'].unique()[::5]

# Add smoothed scratch_relu and scratch_fta as lines
plt.plot(subsampled_goals, smooth_relu, color='brown', linestyle='-', alpha=0.8)
plt.plot(subsampled_goals, smooth_fta, color='green', linestyle='-', alpha=0.8)

# Maintain existing legends
plt.scatter([], [], color='brown', label="ReLU (Scratch)", alpha=0.8)
plt.scatter([], [], color='green', label="FTA (Scratch)", alpha=0.8)
