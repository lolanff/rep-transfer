...
class ARDRQN(NNAgent):
    ...
    def values(self, x: np.ndarray, a, r, carry=None):
        x = np.asarray(x)
        ...
        if len(x.shape) > 1:
            ...
            q, carry, initial_carry = self._values(self.state, x, a, r, carry=carry)
            q = q[0]
    ...
    def policy(self, obs: np.ndarray) -> np.ndarray:
        q, self.carry, _ = self.values(obs, self.last_action_encoded, self.last_reward, carry=self.carry)
        pi = egreedy_probabilities(q, self.actions, self.epsilon)
        return pi

    def ext_policy(self, obs: np.ndarray, last_action_encoded, last_reward, carry: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        q, carry, _ = self.values(obs, last_action_encoded, last_reward, carry=carry)
        pi = egreedy_probabilities(q, self.actions, self.epsilon)
        return pi, q, carry

    @partial(jax.jit, static_argnums=0)
    def _values(self, state: AgentState, x: jax.Array, a: jax.Array, r: jax.Array, carry: jax.Array = None): # type: ignore
        phi = self.phi(state.params, x, a, r, carry=carry)
        return self.q(state.params, phi[0][:, -1]), phi[1][:, -1], phi[2]
    
    @partial(jax.jit, static_argnums=0)
    def _rep_values(self, state: AgentState, x: jax.Array, a: jax.Array, r: jax.Array, carry: jax.Array = None): # type: ignore
        phi = self.phi(state.params, x, a, r, carry=carry)
    ...
    def _loss(self, params: hk.Params, target: hk.Params, batch: CarryBatch, weights: jax.Array):
        ...
        r = batch.r
        gamma = batch.gamma
        last_action_encoded = batch.last_action_encoded
        ...
        phi = self.phi(params, x, last_action_encoded, last_reward, carry=carry, reset=reset, is_target=False)[0]
        phi_p = self.phi(target, xp, action_encoded, r, carry=carryp, reset=reset, is_target=True)[0]
    ...
    def start(self, x: np.ndarray): # type: ignore
        ...
        carry = self.values(x, self.last_action_encoded, self.last_reward)[2]
        pi = self.policy(x)
